# -*- coding: utf-8 -*-
"""Comprehensive unit tests for oracle_profile module."""
...
from ansible_collections.stevefulme1.oracledb.plugins.modules import oracle_profile
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(oracle_profile, "DOCUMENTATION")
        assert len(oracle_profile.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "oracle_profile" in oracle_profile.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in oracle_profile.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in oracle_profile.DOCUMENTATION

    def test_documentation_has_state(self):
        assert "state" in oracle_profile.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(oracle_profile, "EXAMPLES")
        assert len(oracle_profile.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.oracledb" in oracle_profile.EXAMPLES

    def test_return_exists(self):
        assert hasattr(oracle_profile, "RETURN")
        assert len(oracle_profile.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-profile"}
        result = mock_client.create("profile", {"name": "test-profile"})
        assert result["id"] == "new-1"
        assert result["name"] == "test-profile"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-profile", "description": "full test", "enabled": True}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("profile", params)
        assert result["name"] == "full-profile"
        assert result["enabled"] is True

    def test_create_sets_changed_flag(self, mock_client):
        result = {"changed": True, "profile": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent_existing(self, mock_client_existing):
        """Creating an existing resource should not change."""
        existing = mock_client_existing.get("profile", "123")
        assert existing is not None
        result = {"changed": False, "profile": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("profile", "123")
        mock_client_existing.delete.assert_called_once_with("profile", "123")
...
class TestGet:
    ...
    def test_get_existing_resource(self, mock_client_existing):
        result = mock_client_existing.get("profile", "123")
        assert result["id"] == "123"
        assert result["name"] == "existing"

    def test_get_nonexistent_resource(self, mock_client):
        result = mock_client.get("profile", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("profile", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-profile"}
        result = mock_client.update("profile", "123", {"name": "updated-profile"})
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "profile": mock_client_existing.update("profile", "123", {"name": "changed"})}
        assert result["changed"] is True

    def test_update_partial_params(self, mock_client):
        mock_client.update.return_value = {"id": "123", "description": "new desc"}
        result = mock_client.update("profile", "123", {"description": "new desc"})
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("profile")
        assert len(result) == 3

    def test_list_empty(self, mock_client):
        mock_client.list.return_value = []
        assert len(mock_client.list("profile")) == 0
...
class TestErrorHandling:
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("profile", {"invalid_field": "bad"})
...
class TestReturnValues:
    ...
    def test_return_has_resource_key(self):
        result = {"changed": True, "profile": {"id": "1", "name": "test"}}
        assert "profile" in result
        assert isinstance(result["profile"], dict)

    def test_return_resource_has_id(self):
        result = {"changed": True, "profile": {"id": "abc-123"}}
        assert "id" in result["profile"]

    def test_return_on_absent(self):
        result = {"changed": True}
        assert result["changed"] is True
        assert "profile" not in result or result.get("profile") is None or result.get("profile") == {}

    def test_return_unchanged_on_noop(self):
        result = {"changed": False, "profile": {"id": "1"}}
        assert result["changed"] is False
