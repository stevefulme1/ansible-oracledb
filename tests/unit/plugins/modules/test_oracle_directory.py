# -*- coding: utf-8 -*-
"""Comprehensive unit tests for oracle_directory module."""
...
from ansible_collections.stevefulme1.oracledb.plugins.modules import oracle_directory
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(oracle_directory, "DOCUMENTATION")
        assert len(oracle_directory.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "oracle_directory" in oracle_directory.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in oracle_directory.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in oracle_directory.DOCUMENTATION

    def test_documentation_has_state(self):
        assert "state" in oracle_directory.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(oracle_directory, "EXAMPLES")
        assert len(oracle_directory.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.oracledb" in oracle_directory.EXAMPLES

    def test_return_exists(self):
        assert hasattr(oracle_directory, "RETURN")
        assert len(oracle_directory.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-directory"}
        result = mock_client.create("directory", {"name": "test-directory"})
        assert result["id"] == "new-1"
        assert result["name"] == "test-directory"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-directory", "description": "full test", "enabled": True}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("directory", params)
        assert result["name"] == "full-directory"
        assert result["enabled"] is True

    def test_create_sets_changed_flag(self, mock_client):
        result = {"changed": True, "directory": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent_existing(self, mock_client_existing):
        """Creating an existing resource should not change."""
        existing = mock_client_existing.get("directory", "123")
        assert existing is not None
        result = {"changed": False, "directory": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("directory", "123")
        mock_client_existing.delete.assert_called_once_with("directory", "123")
...
class TestGet:
    ...
    def test_get_existing_resource(self, mock_client_existing):
        result = mock_client_existing.get("directory", "123")
        assert result["id"] == "123"
        assert result["name"] == "existing"

    def test_get_nonexistent_resource(self, mock_client):
        result = mock_client.get("directory", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("directory", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-directory"}
        result = mock_client.update("directory", "123", {"name": "updated-directory"})
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "directory": mock_client_existing.update("directory", "123", {"name": "changed"})}
        assert result["changed"] is True

    def test_update_partial_params(self, mock_client):
        mock_client.update.return_value = {"id": "123", "description": "new desc"}
        result = mock_client.update("directory", "123", {"description": "new desc"})
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("directory")
        assert len(result) == 3

    def test_list_empty(self, mock_client):
        mock_client.list.return_value = []
        assert len(mock_client.list("directory")) == 0
...
class TestErrorHandling:
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("directory", {"invalid_field": "bad"})
...
class TestReturnValues:
    ...
    def test_return_has_resource_key(self):
        result = {"changed": True, "directory": {"id": "1", "name": "test"}}
        assert "directory" in result
        assert isinstance(result["directory"], dict)

    def test_return_resource_has_id(self):
        result = {"changed": True, "directory": {"id": "abc-123"}}
        assert "id" in result["directory"]

    def test_return_on_absent(self):
        result = {"changed": True}
        assert result["changed"] is True
        assert "directory" not in result or result.get("directory") is None or result.get("directory") == {}

    def test_return_unchanged_on_noop(self):
        result = {"changed": False, "directory": {"id": "1"}}
        assert result["changed"] is False
