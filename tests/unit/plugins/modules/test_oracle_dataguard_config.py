# -*- coding: utf-8 -*-
"""Comprehensive unit tests for oracle_dataguard_config module."""
...
from ansible_collections.stevefulme1.oracledb.plugins.modules import oracle_dataguard_config
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(oracle_dataguard_config, "DOCUMENTATION")
        assert len(oracle_dataguard_config.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "oracle_dataguard_config" in oracle_dataguard_config.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in oracle_dataguard_config.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in oracle_dataguard_config.DOCUMENTATION

    def test_documentation_has_state(self):
        assert "state" in oracle_dataguard_config.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(oracle_dataguard_config, "EXAMPLES")
        assert len(oracle_dataguard_config.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.oracledb" in oracle_dataguard_config.EXAMPLES

    def test_return_exists(self):
        assert hasattr(oracle_dataguard_config, "RETURN")
        assert len(oracle_dataguard_config.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-dataguard"}
        result = mock_client.create("dataguard", {"name": "test-dataguard"})
        assert result["id"] == "new-1"
        assert result["name"] == "test-dataguard"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-dataguard", "description": "full test", "enabled": True}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("dataguard", params)
        assert result["name"] == "full-dataguard"
        assert result["enabled"] is True

    def test_create_sets_changed_flag(self, mock_client):
        result = {"changed": True, "dataguard": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent_existing(self, mock_client_existing):
        """Creating an existing resource should not change."""
        existing = mock_client_existing.get("dataguard", "123")
        assert existing is not None
        result = {"changed": False, "dataguard": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("dataguard", "123")
        mock_client_existing.delete.assert_called_once_with("dataguard", "123")
...
class TestGet:
    ...
    def test_get_existing_resource(self, mock_client_existing):
        result = mock_client_existing.get("dataguard", "123")
        assert result["id"] == "123"
        assert result["name"] == "existing"

    def test_get_nonexistent_resource(self, mock_client):
        result = mock_client.get("dataguard", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("dataguard", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-dataguard"}
        result = mock_client.update("dataguard", "123", {"name": "updated-dataguard"})
        assert result["name"] == "updated-dataguard"
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "dataguard": mock_client_existing.update("dataguard", "123", {"name": "changed"})}
        assert result["changed"] is True

    def test_update_partial_params(self, mock_client):
        mock_client.update.return_value = {"id": "123", "description": "new desc"}
        result = mock_client.update("dataguard", "123", {"description": "new desc"})
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("dataguard")
        assert len(result) == 3

    def test_list_empty(self, mock_client):
        mock_client.list.return_value = []
        assert len(mock_client.list("dataguard")) == 0
...
class TestErrorHandling:
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("dataguard", {"invalid_field": "bad"})
...
class TestReturnValues:
    ...
    def test_return_has_resource_key(self):
        result = {"changed": True, "dataguard": {"id": "1", "name": "test"}}
        assert "dataguard" in result
        assert isinstance(result["dataguard"], dict)

    def test_return_resource_has_id(self):
        result = {"changed": True, "dataguard": {"id": "abc-123"}}
        assert "id" in result["dataguard"]

    def test_return_on_absent(self):
        result = {"changed": True}
        assert result["changed"] is True
        assert "dataguard" not in result or result.get("dataguard") is None or result.get("dataguard") == {}

    def test_return_unchanged_on_noop(self):
        result = {"changed": False, "dataguard": {"id": "1"}}
        assert result["changed"] is False
