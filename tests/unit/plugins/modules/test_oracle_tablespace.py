# -*- coding: utf-8 -*-
"""Comprehensive unit tests for oracle_tablespace module."""
...
from ansible_collections.stevefulme1.oracledb.plugins.modules import oracle_tablespace
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(oracle_tablespace, "DOCUMENTATION")
        assert len(oracle_tablespace.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "oracle_tablespace" in oracle_tablespace.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in oracle_tablespace.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in oracle_tablespace.DOCUMENTATION

    def test_documentation_has_state(self):
        assert "state" in oracle_tablespace.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(oracle_tablespace, "EXAMPLES")
        assert len(oracle_tablespace.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.oracledb" in oracle_tablespace.EXAMPLES

    def test_return_exists(self):
        assert hasattr(oracle_tablespace, "RETURN")
        assert len(oracle_tablespace.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-tablespace"}
        result = mock_client.create("tablespace", {"name": "test-tablespace"})
        assert result["id"] == "new-1"
        assert result["name"] == "test-tablespace"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-tablespace", "description": "full test", "enabled": True}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("tablespace", params)
        assert result["name"] == "full-tablespace"
        assert result["enabled"] is True

    def test_create_sets_changed_flag(self, mock_client):
        result = {"changed": True, "tablespace": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent_existing(self, mock_client_existing):
        """Creating an existing resource should not change."""
        existing = mock_client_existing.get("tablespace", "123")
        assert existing is not None
        result = {"changed": False, "tablespace": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("tablespace", "123")
        mock_client_existing.delete.assert_called_once_with("tablespace", "123")
...
class TestGet:
    ...
    def test_get_existing_resource(self, mock_client_existing):
        result = mock_client_existing.get("tablespace", "123")
        assert result["id"] == "123"
        assert result["name"] == "existing"

    def test_get_nonexistent_resource(self, mock_client):
        result = mock_client.get("tablespace", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("tablespace", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-tablespace"}
        result = mock_client.update("tablespace", "123", {"name": "updated-tablespace"})
        assert result["name"] == "updated-tablespace"
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "tablespace": mock_client_existing.update("tablespace", "123", {"name": "changed"})}
        assert result["changed"] is True

    def test_update_partial_params(self, mock_client):
        mock_client.update.return_value = {"id": "123", "description": "new desc"}
        result = mock_client.update("tablespace", "123", {"description": "new desc"})
...
class TestList:
    ...
    def test_list_empty(self, mock_client):
        ...
        assert len(mock_client.list("tablespace")) == 0
...
class TestCheckMode:
    ...
    def test_check_mode_create_no_api_call(self, mock_module_check_mode, mock_client):
        ...
        if mock_module_check_mode.check_mode:
            result = {"changed": True, "tablespace": {}}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_client):
        ...
        with pytest.raises(ConnectionError):
            mock_client.get("tablespace", "123")
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("tablespace", {"invalid_field": "bad"})
...
class TestReturnValues:
    ...
    def test_return_has_resource_key(self):
        result = {"changed": True, "tablespace": {"id": "1", "name": "test"}}
        assert "tablespace" in result
        assert isinstance(result["tablespace"], dict)

    def test_return_resource_has_id(self):
        result = {"changed": True, "tablespace": {"id": "abc-123"}}
        assert "id" in result["tablespace"]

    def test_return_on_absent(self):
        result = {"changed": True}
        assert result["changed"] is True
        assert "tablespace" not in result or result.get("tablespace") is None or result.get("tablespace") == {}

    def test_return_unchanged_on_noop(self):
        result = {"changed": False, "tablespace": {"id": "1"}}
        assert result["changed"] is False
